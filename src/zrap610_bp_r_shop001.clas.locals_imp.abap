CLASS lsc_zrap610_r_shop001 DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zrap610_r_shop001 IMPLEMENTATION.


  METHOD save_modified.
    DATA : lt_online_shop_as        TYPE STANDARD TABLE OF zrap610shop001,
           ls_online_shop_as        TYPE                   zrap610shop001,
           lt_online_shop_x_control TYPE STANDARD TABLE OF zrap610_onlineshop_x_001.

    IF create-onlineshop IS NOT INITIAL.
      lt_online_shop_as = CORRESPONDING #( create-onlineshop MAPPING FROM ENTITY ).
      INSERT zrap610shop001 FROM TABLE @lt_online_shop_as.
    ENDIF.
    ...
    IF update IS NOT INITIAL.
      ...
      MODIFY zrap610shop001 FROM TABLE @lt_online_shop_as.
    ENDIF.
    IF delete IS NOT INITIAL.
      LOOP AT delete-onlineshop INTO DATA(onlineshop_delete).
        DELETE FROM zrap610shop001 WHERE order_uuid = @onlineshop_delete-OrderUUID.
        DELETE FROM zrap610shop001 WHERE order_uuid = @onlineshop_delete-OrderUUID.
      ENDLOOP.
    ENDIF.

    IF zrap610_bp_r_shop001=>mapped_purchase_requisition IS NOT INITIAL AND update IS NOT INITIAL.
      LOOP AT zrap610_bp_r_shop001=>mapped_purchase_requisition-purchaserequisition ASSIGNING FIELD-SYMBOL(<purchaserequisition>).
        CONVERT KEY OF i_purchaserequisitiontp FROM <purchaserequisition>-%pid TO DATA(ls_pr_key).
      ...
      LOOP AT update-onlineshop INTO  DATA(ls_online_shop) WHERE %control-OverallStatus = if_abap_behv=>mk-on.
        ...
        UPDATE zrap610shop001 SET purchase_requisition = @ls_pr_key-purchaserequisition,
...
CLASS lhc_onlineshop DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS updateProductDetails FOR DETERMINE ON MODIFY
...
CLASS lhc_onlineshop IMPLEMENTATION.
  ...
  METHOD calculateTotalPrice.
    ...
    DATA total_price TYPE zrap610_r_shop001-TotalPrice.

    " read transfered instances
    READ ENTITIES OF zrap610_r_shop001 IN LOCAL MODE
      ENTITY OnlineShop
    ...
    "update instances
    MODIFY ENTITIES OF zrap610_r_shop001 IN LOCAL MODE
      ENTITY OnlineShop
  ...
  METHOD setInitialOrderValues.
    ...
    SELECT MAX( order_id ) FROM zrap610shop001 INTO @DATA(max_order_id). "active table
    SELECT SINGLE FROM zrap610shop001 FIELDS MAX( order_id ) INTO @DATA(max_orderid_draft). "draft table
  ...
  METHOD updateProductDetails.
    ...
    LOOP AT OnlineShops ASSIGNING FIELD-SYMBOL(<OnlineShop>).
      ...
      SELECT SINGLE * FROM zrap610_i_product_vh WHERE product = @<OnlineShop>-OrderItemID INTO @DATA(product).
  ...
  METHOD checkOrderedItem.
    ...
    DATA products TYPE SORTED TABLE OF zrap610_i_product_vh WITH UNIQUE KEY Product.
    ...
    IF products IS NOT INITIAL.
      ...
      SELECT FROM zrap610_i_product_vh FIELDS product
  ...
  METHOD createPurchaseRequisition.
    DATA: purchase_requisitions      TYPE TABLE FOR CREATE I_PurchaserequisitionTP,
          ...
          update_lines               TYPE TABLE FOR UPDATE zrap610_r_shop001\\OnlineShop,
          update_line                TYPE STRUCTURE FOR UPDATE zrap610_r_shop001\\OnlineShop,
...
*                   purchaserequisitionitemtext
    ...
    zrap610_bp_r_shop001=>mapped_purchase_requisition-purchaserequisition = mapped_create_pr-purchaserequisition.
